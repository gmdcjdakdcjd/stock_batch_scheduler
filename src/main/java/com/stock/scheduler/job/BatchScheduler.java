...
//    // ====================================================
//    @Scheduled(cron = "0 37 23 * * *", zone = "Asia/Seoul") // ✅ 매일 22시에 전부 순차 실행
...
//        // ✅ 통합 리포트 (PDF + 게시글)
//        // ✅ 통합 리포트 (PDF + 게시글)
//        generateAndUploadDailyReport();
//
//    }
//
//
//    private void generateAndUploadDailyReport() {
//        try {
...
//
//            // ✅ 리포트 헤더
//            markdown.append("# 🌍 통합 일일 배치 리포트\n\n")
//                    .append("📅 ").append(today).append("\n\n")
//                    .append("> 🇰🇷 + 🇺🇸 모든 전략 자동 집계 결과\n\n---\n\n");
//
//            // ✅ 하루 전체 범위 설정 (00:00:00 ~ 23:59:59)
//            Timestamp start = Timestamp.valueOf(today.atStartOfDay());
//            Timestamp end = Timestamp.valueOf(today.plusDays(1).atStartOfDay());
//
//            // ✅ 오늘 실행된 모든 JobHistory 조회 (KR + US)
//            List<JobHistory> jobs = jobHistoryRepository.findByDateBetween(start, end);
//
...
//
//            if (jobNames.isEmpty()) {
//                System.err.println("⚠️ 오늘 실행된 배치 기록이 없습니다. 리포트 생성을 건너뜁니다.");
//                return;
//            }
//
//            // ✅ 대시보드 요약
//            String summaryMarkdown = dashboardGenerator.generate(today);
//            markdown.append("## 📊 전체 실행 요약\n\n")
...
//                if (signals.isEmpty())
//                    signals = pythonBatchJob.fetchSignalsFromDBByJob(mappedName, yesterday);
//                if (signals.isEmpty())
//                    continue;
//
...
//                Timestamp sTime = history != null ? history.getStartTime() : new Timestamp(System.currentTimeMillis());
//                Timestamp eTime = history != null ? history.getEndTime() : new Timestamp(System.currentTimeMillis());
//
//                String section = detailGenerator.generate(jobName, signals, sTime, eTime);
//                markdown.append(section).append("\n\n---\n\n");
//            }
//
//            // ✅ PDF 생성
//            String fileName = String.format("TOTAL_Batch_Report_%s", today);
//            File pdfFile = PdfGenerator.generatePdfFromMarkdown(markdown.toString(), fileName);
//
//            // ✅ 게시글 데이터 구성
//            Map<String, Object> board = new HashMap<>();
//            board.put("title", String.format("[자동등록][TOTAL] 🇰🇷🇺🇸 통합 일일 배치 리포트 (%s)", today));
...
//                    "http://localhost:8090/api/board/auto-with-file",
//                    new HttpEntity<>(body, headers),
//                    String.class
//            );
//
//            System.out.println("✅ [TOTAL 리포트 게시 완료]");
//            System.out.println("📄 업로드 결과: " + response);
//
//        } catch (Exception e) {
//            System.err.println("❌ TOTAL 리포트 생성 실패: " + e.getMessage());
//            e.printStackTrace();
//        }
//    }
//
